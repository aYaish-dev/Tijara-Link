  ...
  return (
    <main className="p-6 space-y-4">
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs text-gray-500">{new Date(order.createdAt).toLocaleString()} • {order.id}</p>
      </div>
      ...
      <section className="space-y-2">
        ...
        {order.escrow ? (
          <div className="border rounded p-3 text-sm">
            <p>
              Held: {order.escrow.currency} {(order.escrow.heldMinor/100).toFixed(2)} — Released:{" "}
              {order.escrow.released ? "Yes" : "No"}
            </p>
            {!order.escrow.released && (
              <form onSubmit={async (e) => {
                e.preventDefault();
                await fetch(`http://localhost:3001/orders/${order.id}/escrow/release`, { method: "POST" });
                location.reload();
              }}>
                <button type="submit" className="mt-2 px-3 py-1.5 rounded border">Release</button>
              </form>
            )}
          </div>
        ) : (
        ...
        <div className="border rounded p-3 text-sm">
          <form className="flex gap-2 flex-wrap items-center"
                onSubmit={async (e) => {
                  ...
                  const tracking = (form.elements.namedItem("tracking") as HTMLInputElement).value || "TRK-CLIENT";
            ...
            <button type="submit" className="px-3 py-1.5 rounded border">Add Shipment</button>
          </form>
        </div>
            ...
            <li key={s.id} className="border rounded p-3 text-sm">
              ...
              <div className="mt-2 flex gap-2">
                ...
                <button
                  className="px-2 py-1 rounded border"
                  onClick={async () => {
                    const payload = { data: { hsCode: "7208.38", docs: ["invoice.pdf"] }, status: "SUBMITTED" };
      ...
      <section className="space-y-2">
        ...
        <form className="border rounded p-3 text-sm"
              onSubmit={async (e) => {
                e.preventDefault();
                const form = e.target as HTMLFormElement;
          ...
          <input name="rating" type="number" min="1" max="5" defaultValue={5} className="border px-2 py-1 rounded mr-2" />
          <input name="comment" placeholder="excellent & on-time" className="border px-2 py-1 rounded mr-2 w-64" />
          <button type="submit" className="px-3 py-1.5 rounded border">Save</button>
        </form>
      </section>
    </main>
  );
